...
bangalore_called = [call[1] for call in bangalore_calls]

called_numbers = []

for call in bangalore_called:
    if re.search("\(\w+\)", call):
        called_numbers.append(re.search(r'(\(.*?\))', call).group(1))
    elif re.search("^[7-9]+\w+\s\w+", call):
...
for call in calls:
    if call[0].startswith("(080)") == True:
        if call[1].startswith("(080)") == True:
            total_calls.append(call[1]) 

percentage = round(len(total_calls)/ len(bangalore_calls), 4) * 100
